...
/** Small cart count from localStorage (optional) */
function useCartCount() {
  ...
  }, [pathname]); // recompute on route change
  return count;
}
    ...
    <header className="mc-navbar shadow-sm sticky-top bg-white">
      <nav className="container d-flex align-items-center justify-content-between py-2">
        ...
        <Link to="/" className="brand d-flex align-items-center gap-2" onClick={closeOnNav}>
          {/* Simple heart/med logo (inline SVG) */}
        ...
        <div className={`nav-links ${open ? "show" : ""}`}>
          <ul className="list-unstyled d-lg-flex align-items-center gap-lg-3 m-0">
            ...
            {/* Doctors (non-navigating placeholder for now) */}
            <li><NavLink
                to="/findDoctors"
                className={({ isActive }) =>
                  `nav-link ${isActive ? "active" : ""}`
            ...
            <li><span className="nav-link disabled-link">Contact Us</span></li>
          </ul>
        </div>
        ...
        <div className="actions d-flex align-items-center gap-2">
          ...
          {/* Login button */}
          <Link to="/login" className="btn btn-brand" onClick={closeOnNav}>
            Login
          </Link>
        </div>
      </nav>
